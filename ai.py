...
WHAT HEALTHCHECKS ARE: Healthchecks run BEFORE the watchdog retries a failed cron. If any check fails, the retry is SKIPPED and a "dependency unhealthy" alert fires. They exist to prevent retry-loops against an OOM model server, an upstream-API outage, or a network split.

WHAT TO CHECK: ONLY external network services. Ask yourself:
   "What HTTP service, API, or model endpoint must be RESPONDING right now
    for this cron's retry to have any chance of succeeding?"
That is what to check, and almost always with type=http_health.

WHAT NOT TO CHECK (these are NEVER valid healthchecks):
- Script files (.py / .sh / .js / .ts): they are static code. Their
  presence is a deploy concern, not a runtime concern.
- Workspace directories: mtime is unreliable and the cron's workspace
  is a local concern, not a dependency.
- Configuration files (config.json, .env, etc.): static, do not go offline.
- The cron's OWN output files: that's what predicates verify, not healthchecks.
- Anything that doesn't change minute-by-minute and doesn't have an
  uptime/availability concern.
- The watchdog itself.

PREFER http_health. The other predicate types exist but are rarely
appropriate for healthchecks. Only use file_mtime if you are CERTAIN the
file is regenerated by an external process you cannot otherwise probe
(e.g. a data-feed file written by a separate ingestion daemon).

OUTPUT: 0-3 healthchecks targeting the LIKELIEST external services the
cron depends on. **An empty array `[]` is the correct answer when the
cron has no clear external dependencies.** Do not invent placeholder
checks just to fill space.

PREDICATE SCHEMAS:

- http_health  (use this ~95% of the time)
  {"type":"http_health","url":"...","timeout_seconds":N (optional, default 5),"expected_status":N (optional, default 200),"description":"..."}

- file_mtime  (rare — only for externally-refreshed data files)
...
[
  {"type":"http_health","url":"http://localhost:8100/v1/models","timeout_seconds":3,"description":"vLLM model endpoint must be responding before we retry"},
...
EXAMPLE — for a cron with no external services (e.g. a cleanup task that only touches local files):
